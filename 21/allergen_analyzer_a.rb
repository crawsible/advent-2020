...
class AllergenAnalyzer
  ...
  def nonallergenic_appearances_count
    @foods.map(&:ingredients).flatten.count do |ingredient|
      nonallergenic_ingredients.include?(ingredient)
    end
  end
...
puts analyzer.nonallergenic_appearances_count
