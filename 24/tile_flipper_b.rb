...
class Tile
  ...
  def adjacent_black_tile_count
    @directions_tiles.values.count { |tile| tile.side == :black }
  end
...
class Floor
  ...
  def flip
    expand if @outer_tiles.any? { |tile| tile.side == :black }
    tiles_to_flip = []

    @@tiles.each do |tile|
      case tile.side
      when :white
        tiles_to_flip << tile if tile.adjacent_black_tile_count == 2
      when :black
        tiles_to_flip << tile if !(1..2).include?(tile.adjacent_black_tile_count)
      end
    end
